...
<h2 class="head">Logo &bull; FavIcon</h2>
...
<table class="table">
    ...
    <fieldset>
         <form action="<?=$url.'!/'.$link[1].'/'.$link[2]?>/enviar" method="post" enctype="multipart/form-data">
            ...
            <input id="file" name="file" type="file" autocomplete="" required="required" class="form-control" accept="image/*" >
             <span class="input-group-addon" id="basic-addon2">Somente Imagens (PNG/JPG/GIF) - 64x64 pixel</span>
            </div>
            <br>
  ...
  <tr>
    ...
    <fieldset>
      ...
      <div>
		...
		@mkdir( $__pastaFotos.'favicon', 0777 );
		@chmod( $__pastaFotos.'favicon', 0777 );
			...
			if( !empty($_FILES['file']['name']) ):
			   ...
			   if(move_uploaded_file($_FILES['file']['tmp_name'], $__pastaFotos . "/favicon/" . $NomeArquivoBanner ))
			   {					
